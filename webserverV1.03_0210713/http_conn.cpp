...
bool http_conn::read()
{
    ...
    while(true)
    {
        ...
        if(bytes_read == -1)
        {
            ...
            {
                if(errno == EAGAIN) printf("read() EAGAIN\n");
                if(errno == EWOULDBLOCK) printf("read() EWOULDBLOCK\n");
                break;
            }
            return false;
        }
...
bool http_conn::write()
{
    ...
    while(1)
    {
        ...
        if (temp < 0)
        {
            ...
            if (errno == EAGAIN)
            {
                printf("[EAGAIN]---------write() temp < 0--------\n");
                modfd(m_epollfd, m_sockfd, EPOLLOUT);
                return true;
            }
            printf("[other error]---------write() temp < 0--------\n");
            unmap();
            return false;
        }
        //读了个temp字节数的文件
