...
void http_conn::init(int socket, const sockaddr_in & addr)
{
    m_sockfd = sockfd;
    m_address = addr;
...
bool http_conn::read()
{
    ...
    while(true)
    {
        bytes_read = recv(m_sockfd, m_read_buf, sizeof(m_read_buf));
        if(bytes_read == -1)
        {
...
bool http_conn::add_response(const char* format, ...)
{
    ...
    int len = vsnprintf(m_write_buf + m_write_idx, WRITE_BUFFER_SIZE - 1 - m_write_idx, format, arg_list);//将可变参数写到第一个参数的内存里，写的长度为第二个参数
...
{
    return add_response("Connection:%s",(m_linger == true)?"keep-alive":"close");
}
...
bool http_conn::process_write(HTTP_CODE ret)
{
    switch (ret)
    {
        ...
        case FILE_REQUEST://请求文件
        {
            ...
            if(m_file_stat.st_mode != 0)
            {
                add_headers(m_file_stat.st_size);
                m_iv[0].iov_base = m_write_buf;
                m_iv[0].iov_len = m_write_idx;
                m_iv[1].iov_base = m_file_address;
                m_iv[1].iov_len = m_file_stat.st_mode;
                m_iv_count = 2;
                return true;
            }
            else
...
void http_conn::process()
{
    ...
    {
        nodfd(m_epollfd, m_sockfd, EPOLLIN);
        return;
    }
